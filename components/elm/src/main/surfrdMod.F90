...
!-----------------------------------------------------------------------
    ...
    if (.not.use_ats) then
      ...
      wt_lunit(begg:endg,:,istsoil) = 1._r8 ! hardcode to be all istsoil for now - RPF
      wt_lunit(begg:endg,:,istcrop) = 0._r8
      wt_nat_patch(begg:endg,:,:) = 1._r8
      wt_polygon(begg:endg,:,:) = 0._r8 ! polygonal tundra
    endif
    ...
    if (use_ats .and. (crop_prog .or. create_crop_landunit)) then
      call endrun( msg=' ERROR: use_ats does not work with crops yet - RPF surfrdMod.F90'//&
      errMsg(__FILE__, __LINE__))
    endif
