module ELM_ATS_InterfaceMod
  ...
  type, public :: elm_ats_interface_type
   ...
   contains
     ...
     procedure, public :: delete        => ats_delete
...
contains
  ...
  subroutine ats_advance(this, dt, do_vis, do_chkp)
    implicit none
    class(elm_ats_interface_type) :: this
    real(r8), intent(in) :: dt                                      ! one ELM timestep interval (in seconds)
    logical, intent(in) :: do_vis                     ! instruct ATS output data
    logical, intent(in) :: do_chkp                    ! instruct ATS output checkpoint
    !
    ! local variables
    logical(KIND=C_BOOL) :: do_vis_c = .false.
    logical(KIND=C_BOOL) :: do_chkp_c = .false.
    do_vis_c = do_vis
    do_chkp_c = do_chkp

    call ats_advance_c(this%ptr, dt, do_vis_c, do_chkp_c)
